...
import { PenguinCordDevsById, EquicordDevsById, VencordDevsById } from "@utils/constants";
...
function ContributorModal({ user, modalProps }: { user: User; modalProps: RenderModalProps; }) {
    ...
    const plugins = useMemo(() => {
        ...
        const pluginsByAuthor = (VencordDevsById[user.id] || PenguinCordDevsById[user.id] || EquicordDevsById[user.id])
            ? allPlugins.filter(p => p.authors.includes(VencordDevsById[user.id] || PenguinCordDevsById[user.id] || EquicordDevsById[user.id]))
            : allPlugins.filter(p =>
    ...
    const ContributedHyperLink = <Link href="https://github.com/penguinrust/Penguincord">contributed</Link>;
