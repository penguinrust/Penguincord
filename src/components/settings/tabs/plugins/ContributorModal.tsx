...
import { PenguincordDevsById, VencordDevsById } from "@utils/constants";
...
function ContributorModal({ user, modalProps }: { user: User; modalProps: RenderModalProps; }) {
    ...
    const plugins = useMemo(() => {
        ...
        const pluginsByAuthor = (VencordDevsById[user.id] || PenguincordDevsById[user.id])
            ? allPlugins.filter(p => p.authors.includes(VencordDevsById[user.id] || PenguincordDevsById[user.id]))
            : allPlugins.filter(p =>
