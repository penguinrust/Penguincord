...
console.log("[Penguincord] Starting up...");
...
if (!IS_VANILLA) {
    ...
    if (!IS_VESKTOP && !IS_EQUIBOP) {
        try {
            ...
            console.error("[Penguincord] Failed to install host update hook", err);
        }
    }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "Penguincord");
    ...
    console.log("[Penguincord] Running in vanilla mode. Not loading Penguincord");
}

console.log("[Penguincord] Loading original Discord app.asar");
