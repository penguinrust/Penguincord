    ...
    try {
        ...
        for (let i = undo.length - 1; i >= 0; i--) {
            try {
                ...
                console.error("[Penguincord] Rollback step failed", cleanupErr);
            }
        }
        throw err;
    }
    ...
    try {
        for (const name of readdirSync(discordPath)) {
            ...
            try {
                ...
                console.error("[Penguincord] Skipping unreadable sibling", name, statErr);
                continue;
            }
            if (!isDir) continue;
        ...
        console.error("[Penguincord] Failed to scan for sibling versions", err);
        return null;
    }
