...
export function isPenguincordGuild(id: string | null | undefined, isGuildId: boolean = false): boolean {
    if (!id) return false;
    if (isGuildId) return id === PC_GUILD_ID;
    const channel = ChannelStore.getChannel(id);
    if (!channel) return false;
    return channel.guild_id === PC_GUILD_ID;
}

export function isPenguincordSupport(userId: string | null | undefined): boolean {
    if (!userId) return false;

    const member = GuildMemberStore.getMember(PC_GUILD_ID, userId);
    if (!member) return false;
    return member.roles.includes(PENGUINCORD_HELPERS) || false;
}
